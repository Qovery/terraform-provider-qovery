...
func TestNewCredentialsAzureService(t *testing.T) {
	t.Parallel()

	testCases := []struct {
		TestName      string
		Repository    credentials.AzureRepository
		ExpectedError error
	}{
		{
		...
		{
			TestName:   "success",
			Repository: &mocks_test.CredentialsAzureRepository{},
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.TestName, func(t *testing.T) {
			...
			svc, err := services.NewCredentialsAzureService(tc.Repository)
...
func TestCredentialsAzureService_Create(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		SetupMock      func(*mocks_test.CredentialsAzureRepository)
		ExpectedError  error
	}{
		{
			...
			SetupMock:     func(m *mocks_test.CredentialsAzureRepository) {},
			ExpectedError: credentials.ErrFailedToCreateAzureCredentials,
		},
		{
		...
		},
		{
			...
			SetupMock: func(m *mocks_test.CredentialsAzureRepository) {
				creds := &credentials.AzureCredentials{
					...
					AzureSubscriptionId:      gofakeit.UUID(),
					AzureTenantId:            gofakeit.UUID(),
					AzureApplicationId:       gofakeit.UUID(),
					AzureApplicationObjectId: gofakeit.UUID(),
				}
				m.EXPECT().Create(mock.Anything, mock.Anything, mock.Anything).
					Return(creds, nil)
			},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.TestName, func(t *testing.T) {
			...
			mockRepo := &mocks_test.CredentialsAzureRepository{}
			tc.SetupMock(mockRepo)

			svc, err := services.NewCredentialsAzureService(mockRepo)
			assert.NoError(t, err)
...
func TestCredentialsAzureService_Get(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			SetupMock:      func(m *mocks_test.CredentialsAzureRepository) {},
			ExpectedError:  credentials.ErrFailedToGetAzureCredentials,
		},
		{
...
func TestCredentialsAzureService_Update(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			SetupMock:     func(m *mocks_test.CredentialsAzureRepository) {},
			ExpectedError: credentials.ErrFailedToUpdateAzureCredentials,
		},
		{
			TestName:       "fail_with_empty_credentials_id",
			OrganizationID: uuid.NewString(),
...
func TestCredentialsAzureService_Delete(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			CredentialsID:  uuid.NewString(),
			SetupMock:      func(m *mocks_test.CredentialsAzureRepository) {},
			ExpectedError:  credentials.ErrFailedToDeleteAzureCredentials,
		},
		{
			TestName:       "fail_with_empty_credentials_id",
			OrganizationID: uuid.NewString(),
