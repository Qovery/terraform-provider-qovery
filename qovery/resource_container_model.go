...
func (cont Container) ExternalSecretFileList() ExternalSecretFileList {
	return toExternalSecretFileList(cont.ExternalSecretFiles)
}
...
func (cont Container) toUpsertServiceRequest(state *Container) *container.UpsertServiceRequest {
	...
	return &container.UpsertServiceRequest{
		...
		ExternalSecretFiles:          cont.ExternalSecretFileList().diffRequest(stateExternalSecretFiles),
	}
}
...
func convertDomainContainerToContainer(ctx context.Context, state Container, container *container.Container) Container {
	...
	return Container{
		...
		ExternalSecrets:              convertDomainExternalSecretsToExternalSecretList(container.ExternalSecrets, state.ExternalSecrets).toTerraformSet(ctx),
		ExternalSecretFiles:          convertDomainExternalSecretFilesToExternalSecretFileList(container.ExternalSecretFiles, state.ExternalSecretFiles).toTerraformSet(ctx),
	}
}
