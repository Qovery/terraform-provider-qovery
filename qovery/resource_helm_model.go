...
func convertDomainHelmToHelm(ctx context.Context, state Helm, helm *helm.Helm) Helm {
	...
	return Helm{
		...
		ExternalSecrets:              convertDomainExternalSecretsToExternalSecretList(helm.ExternalSecrets, state.ExternalSecrets).toTerraformSet(ctx),
		ExternalSecretFiles:          convertDomainExternalSecretFilesToExternalSecretFileList(helm.ExternalSecretFiles, state.ExternalSecretFiles).toTerraformSet(ctx),
	}
}
