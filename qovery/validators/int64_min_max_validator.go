...
func (v Int64MinMaxValidator) Validate(ctx context.Context, req tfsdk.ValidateAttributeRequest, resp *tfsdk.ValidateAttributeResponse) {
	...
	if number.Value < v.Min || number.Value > v.Max {
		resp.Diagnostics.AddAttributeError(
			req.AttributePath,
			"Invalid Number Value",
			fmt.Sprintf("Number value must be between (inclusive) %d and %d, got: %d.", v.Min, v.Max, number.Value),
		)
		return
	}
}
