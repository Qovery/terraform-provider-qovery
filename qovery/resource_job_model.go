...
func convertDomainJobToJob(ctx context.Context, state Job, job *job.Job) Job {
	...
	return Job{
		...
		Source:                       &source,
		Schedule:                     &schedule,
		BuiltInEnvironmentVariables:  convertDomainVariablesToEnvironmentVariableList(ctx, job.BuiltInEnvironmentVariables, variable.ScopeBuiltIn, "BUILT_IN").toTerraformSet(ctx),
