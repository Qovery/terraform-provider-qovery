...
func convertDomainJobToJob(ctx context.Context, state Job, job *job.Job) Job {
	...
	return Job{
		...
		Source:                       &source,
		Schedule:                     &schedule,
		BuiltInEnvironmentVariables:  convertDomainVariablesToEnvironmentVariableList(job.BuiltInEnvironmentVariables).toTerraformList(ctx),
		...
		ExternalSecrets:              convertDomainExternalSecretsToExternalSecretList(job.ExternalSecrets, state.ExternalSecrets).toTerraformSet(ctx),
		ExternalSecretFiles:          convertDomainExternalSecretFilesToExternalSecretFileList(job.ExternalSecretFiles, state.ExternalSecretFiles).toTerraformSet(ctx),
	}
}
